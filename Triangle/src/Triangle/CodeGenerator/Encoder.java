...
public final class Encoder implements Visitor {
  ...
  public Object visitWhileCommand(WhileCommand ast, Object o) {
    ...
    public Object visitForCommand(ForCommand ast, Object o) {
      Frame frame = (Frame) o;
      ast.V.visit(this, frame);                                             // dirección de i
      ast.E1.visit(this, frame);                                            // valor inicial
      Boolean isTo = ast.isTo;

      encodeStore(ast.V, new Frame (frame, 1), 1);           // guardar la variable en el stack

      int loopTestJump = nextInstrAddr;                                         // dirección de la condición
      emit(Machine.JUMPop, 0, Machine.CBr, 0);                         // salto hacia prueba
      int loopStart = nextInstrAddr;                                            // dirección del cuerpo
      ast.C.visit(this, frame);                                             // Cuerpo del for

      
      encodeFetch(ast.V, frame, 1);                                     // Sacar valor de la variable
      // i++
      if (isTo) 
      ...
      // Revisar condición del for
      patch(loopTestJump, nextInstrAddr);
      encodeFetch(ast.V, frame, 1);
      ast.E2.visit(this, frame);                                            // valor final (to)
      
      if (isTo) 
          emit(Machine.CALLop, Machine.SBr, Machine.PBr, Machine.leDisplacement); // i <= E2
      else
          emit(Machine.CALLop, Machine.SBr, Machine.PBr, Machine.geDisplacement); // i >= E2
      
      emit(Machine.JUMPIFop, Machine.trueRep, Machine.CBr, loopStart); // Si verdadero, saltar al inicio del cuerpo
      return null;
    }

    public Object visitMatchCommand(MatchCommand ast, Object o) {
        Frame frame = (Frame) o;
        List<Integer> patchExecuted = new ArrayList<>();
        int nextCaseAddr;
        int jumpToEnd;
        for (Map.Entry<ArrayList<Expression>, Command> entry: ast.MAP.entrySet()){ //Para cada case
            for (Expression caseExpr : entry.getKey()){// Para cada exzpresion que el case acepta
                //Evaluar expresiones del match(ast.E) y caseExpr
                ast.E.visit(this, frame);
                caseExpr.visit(this,frame);
                emit(Machine.LOADLop, 0,0,1);
                //Comparar los valores del match y caseExpr
                emit(Machine.CALLop, Machine.LBr, Machine.PBr, Machine.eqDisplacement);
                //Si no es igual, salta a sig instruccion
                nextCaseAddr = nextInstrAddr;
                emit(Machine.JUMPIFop, Machine.falseRep, Machine.CBr, nextCaseAddr);
                //Si no salto, ejecuta el comando (entry.getValue())
                entry.getValue().visit(this, frame);                            
                // Saltar el resto del match (ya se ejecutó un case)
                jumpToEnd = nextInstrAddr;
                patchExecuted.add(jumpToEnd); //Para luego parchear esto
                emit(Machine.JUMPop, 0, Machine.CBr, 0);
                //parcheamos la direccion de la siguiente expresion en el caso
                patch(nextCaseAddr, nextInstrAddr);
            }
        }
        if(ast.OTHERWISE != null){
            ast.OTHERWISE.visit(this, frame);   // Si no se ejecuto ningun case, vamos al otherwise
        }
        for (int addr : patchExecuted) {        //Parcheamos las label que decian que se ejecuto un case
  ...
  public Object visitRepeatCommand(RepeatCommand ast, Object o) {
    Frame frame = (Frame) o;
    int loopAddr;                                                               
    loopAddr = nextInstrAddr;                                                   // Posición del cuerpo del ciclo
    ast.C.visit(this, frame);                                               // Visita el cuerpo
    ast.E.visit(this, frame);                                               // Visita la expresión en la condición
    emit(Machine.JUMPIFop, Machine.falseRep, Machine.CBr, loopAddr);   // Hace un salto al cuerpo si la condiciónd da falso
    return null;
  }
  // Expressions
  ...
  public Object visitMatchExpression(MatchExpression ast, Object o) {
        ...
        int nextCaseAddr;
        int jumpToEnd;
        for (Map.Entry<ArrayList<Expression>, Expression> entry: ast.MAP.entrySet()){ //Para cada case
            for (Expression caseExpr : entry.getKey()){// Para cada exzpresion que el case acepta
                //Evaluar expresiones del match(ast.E) y caseExpr
                ast.E.visit(this, frame);
                caseExpr.visit(this,frame);
                emit(Machine.LOADLop, 0,0,1);
                emit(Machine.CALLop, Machine.LBr, Machine.PBr, Machine.eqDisplacement); //Comparar los valores del match y caseExpr
                nextCaseAddr = nextInstrAddr;
                emit(Machine.JUMPIFop, Machine.falseRep, Machine.CBr, nextCaseAddr);//Si no es igual, salta a sig instruccion
                entry.getValue().visit(this, frame); //Si no salto, evalua la expresion (entry.getValue())                          
                // Saltar el resto del match (ya se ejecutó un case)
                jumpToEnd = nextInstrAddr;
                patchExecuted.add(jumpToEnd); //Para luego parchear esto
                emit(Machine.JUMPop, 0, Machine.CBr, 0);
        ...
        // Si no se ejecuto ningun case, vamos al otherwise
        Integer valSize = (Integer) ast.OTHERWISE.visit(this, frame);
        //Parcheamos las label que decian que se ejecuto un case
        for (int addr : patchExecuted) {
            patch(addr, nextInstrAddr);
        }
        return valSize;
    }
...
public Object visitClassTypeDenoter(ClassTypeDenoter ast, Object o) {   
    ...
    if (ast.entity == null) { // Calcular el tamaño del tipo
        ...
        //Obtener el tamaño de la clase sin visitar funciones
        //Se pasa un frame de mentiras
        Integer bodySize = (Integer) ast.body.visit(this, new Frame());       
        typeSize += bodySize;
