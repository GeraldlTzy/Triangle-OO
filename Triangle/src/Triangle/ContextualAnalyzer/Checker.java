...
public final class Checker implements Visitor {
  ...
  public Object visitWhileCommand(WhileCommand ast, Object o) {
    ...
    public Object visitForCommand(ForCommand ast, Object o) {
      // Verifica que la variable de control esté declarada
      TypeDenoter varType = (TypeDenoter) ast.V.visit(this, null);
      if (!varType.equals(StdEnvironment.integerType)) {
        reporter.reportError("La variable del 'for' debe ser de tipo Integer", "", ast.V.position);
      }

      // Verifica que las expresiones sean Integer
      TypeDenoter e1Type = (TypeDenoter) ast.E1.visit(this, null);
      TypeDenoter e2Type = (TypeDenoter) ast.E2.visit(this, null);

      if (!e1Type.equals(StdEnvironment.integerType)) {
        reporter.reportError("La expresión inicial del 'for' debe ser Integer", "", ast.E1.position);
      }
      if (!e2Type.equals(StdEnvironment.integerType)) {
        reporter.reportError("La expresión final del 'for' debe ser Integer", "", ast.E2.position);
      }

      // Verifica el cuerpo del for
      ast.C.visit(this, null);
  ...
  public Object visitRepeatCommand(RepeatCommand ast, Object o){
    ...
    TypeDenoter condType = (TypeDenoter) ast.E.visit(this, o);

    if (!condType.equals(StdEnvironment.booleanType)) {
        reporter.reportError("La condición del repeat-until debe ser de tipo booleano", "", ast.E.position);
    }
    return null;
  }
    
  public Object visitMatchCommand(MatchCommand ast, Object o) {
      //Verificar que el identificador del match sea Integer o bool
      TypeDenoter eType = (TypeDenoter) ast.E.visit(this, null);
      if (!(eType.equals(StdEnvironment.booleanType) || eType.equals(StdEnvironment.integerType))) {
          reporter.reportError("Boolean or Integer expression expected here", "", ast.E.position);
      }
      
      // Verificar cada CASE
      for (Map.Entry<ArrayList<Expression>,Command> entry : ast.MAP.entrySet()){
          //Verificamos validez de cada expresion en el case
          //Tambien verificamos que su tipo sea el mismo que la expresion del match
          for (Expression caseExpr:entry.getKey()){
              TypeDenoter exprCaseType = (TypeDenoter) caseExpr.visit(this, null);
              if(!exprCaseType.equals(eType)){
                  reporter.reportError("Case value type is not equal to match expresion type", "", caseExpr.position);
              }
          }
          //Verificamos la validez de el comando en cada case
          entry.getValue().visit(this, null);
      }
      
      //Verificar el OTHERWISE
  ...
  public Object visitMatchExpression(MatchExpression ast, Object o) {
      ...
      // Verificar cada CASE
      for (Map.Entry<ArrayList<Expression>,Expression> entry : ast.MAP.entrySet()){
          //Verificamos validez de cada expresion en el case
          //Tambien verificamos que su tipo sea el mismo que la expresion del match
          for (Expression caseExpr:entry.getKey()){
              TypeDenoter exprCaseType = (TypeDenoter) caseExpr.visit(this, null);
          ...
          //Verificamos la validez de la expression en cada case
          entry.getValue().visit(this, null);
      }
